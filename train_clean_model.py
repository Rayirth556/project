...
===================
Trains XGBoost model without leaked features and constant features.
Removes academic_background_tier (data leakage) and constant features.
"""
...
DEFAULT_MODEL_PATH = MODEL_DIR / "pdr_xgb_clean.json"
DEFAULT_CALIBRATION_PATH = MODEL_DIR / "pdr_xgb_clean_calibration.json"
DEFAULT_FEATURE_AUDIT_PATH = MODEL_DIR / "feature_readiness_audit_clean.json"

# Features to remove due to data leakage or zero variance
LEAKED_FEATURES = ["academic_background_tier"]  # Data leakage
CONSTANT_FEATURES = ["operating_cashflow_survival_flag", "turnover_inflation_spike"]  # Zero variance
...
def load_data() -> tuple[pd.DataFrame, pd.Series]:
    ...
    removed_count = original_features - X.shape[1]
    
    print(f"  Removed {removed_count} problematic features:")
    for feat in REMOVED_FEATURES:
        if feat in X.columns:
            print(f"    - {feat}")
    
    print(f"  Final dataset: {len(X):,} samples x {X.shape[1]} clean features")
    return X, y
...
def build_feature_readiness_audit(X: pd.DataFrame) -> dict:
    """Summarize feature quality for clean dataset."""
    audit: dict[str, dict] = {}
    for col in X.columns:
    ...
    *,
    ...
    if imbalance == "scale_pos_weight":
        ...
        try:
            ...
            raise ImportError("imbalanced-learn is required for --imbalance smote")
    ...
    # Threshold selection (on VAL), then evaluation on TEST
    ...
    print(f"\n  Success: Clean model saved -> {model_path}")

    # Save feature importance
    fi_path = model_path.parent / "feature_importance_clean.json"
    fi_path.write_text(json.dumps(importance_sorted, indent=2))

    # Save training report
    report_path = model_path.parent / "training_report_clean.txt"
    with open(report_path, "w") as f:
        f.write(f"PDR Pipeline - Layer 3 Clean Model Training Report\n")
        f.write(f"=" * 55 + "\n\n")
        ...
        f.write(f"Removed Features: {REMOVED_FEATURES}\n\n")
        f.write(f"Metrics:\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Layer 3 XGBoost model WITHOUT leaked features.")
    ...
    parser.add_argument("--threshold-strategy", choices=["youden", "f1", "f05", "fixed"], default="f05",
                        help="How to choose classification threshold (default: f05)")
    ...
    parser.add_argument("--calibrate", choices=["none", "platt"], default="platt",
                        help="Optional probability calibration (default: platt)")
    ...
    print("\n" + "=" * 62)
    print("  PDR Pipeline - Layer 3 Clean Model Training")
    print("=" * 62)

    print("\n[1/4] Loading and cleaning features ...")
    X, y = load_data()
    ...
    print("\n[3/4] Using default hyperparameters (no tuning for clean model) ...")
    ...
    print("\n" + "=" * 62)
    print("  Success: Clean Model Training Complete!")
    ...
    print(f"\n  To run inference with this clean model:")
